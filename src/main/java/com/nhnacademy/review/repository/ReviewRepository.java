...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT new com.nhnacademy.review.domain.dto.ReviewStatsResponse(r.isbn, AVG(r.rating), COUNT(r)) " +
            "FROM Review r GROUP BY r.isbn HAVING COUNT(r) >= :minReviewCount ORDER BY AVG(r.rating) DESC")
    List<ReviewStatsResponse> findBooksByAverageRatingDescWithMinReviews(@Param("minReviewCount") long minReviewCount);

    @Query("SELECT new com.nhnacademy.review.domain.dto.ReviewStatsResponse(r.isbn, AVG(r.rating), COUNT(r)) " +
            "FROM Review r GROUP BY r.isbn ORDER BY COUNT(r) DESC")
